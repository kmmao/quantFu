...
class QuantFuPusher:
    ...
    def push_trade(self, account_id: str, symbol: str, direction: str, offset: str,
                   volume: int, price: float, order_id: str = None, commission: float = 0) -> bool:
        """推送成交数据"""
        if not self.enable:
            return True
    ...
    def push_snapshot(self, account_id: str, positions: list) -> bool:
        """推送持仓快照"""
        if not self.enable:
            return True
        ...
        try:
            ...
            response = requests.post(
                f"{self.api_url}/api/position_snapshots",
                json=snapshot_data,
                headers={"Content-Type": "application/json", "X-API-Key": self.api_key},
                timeout=3
            )
...
# QuantFu 快照推送控制 (每10分钟推送一次)
quantfu_last_snapshot_bar = 0  # 上次快照推送的bar
            ...
            if ret_enter == 0 or ret_enter == -2:
                ...
                try:
                    quantfu_pusher.push_trade(
                        ...
                        commission=order_num * 5  # 根据实际手续费调整
                    )
                except:
                    pass  # 推送失败不影响交易
                # ========================================================
